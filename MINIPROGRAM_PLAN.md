...
## 0.5 体验版零备案路线（2026-09-12 采纳，当前实施路线）

用户群体就是家人（≤15 人），**正式发布与备案整条链路取消**，改走微信「体验版」长期使用：

- **不备案、不迁云、不配域名**：后端继续用家里 NAS 上的现有 Docker 服务。
- 体验版真机打开「开发调试」后跳过服务器域名白名单校验，`wx.request` 直连
  局域网地址（听写场景本来就在家）。
- 鉴权 / 限流（§4.1、§4.2）随公网暴露面一起消失，本路线不做。
- 小程序端已实现：`miniprogram/`（原生框架，无构建依赖），操作手册见
  `miniprogram/README.md`。
- 后端唯一改动：`POST /api/worksheet` 增加 `application/json` + base64 data URL
  入参（`lib/api/dataurl.ts`），与 multipart 并存，Web 版不受影响。
- 已知代价：不能被搜索 / 正式分享给非体验成员、界面带体验版角标、
  长期零活跃的账号可能被回收（偶尔上传新版即可）。

原 §3–§6 的合规与产品化流程**保留为将来公开发布时的路线图**，本路线不执行。
