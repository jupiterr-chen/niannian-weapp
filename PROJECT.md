...
| D-27 | 字号下限按**手机实测观感**定，不按桌面 | 关键认知：手机竖屏 390px 宽时 `vmin = 3.9px`，`clamp(20px, 2.6vmin, 24px)` 算出 10px，**永远取下限**。也就是说手机上看到的恒等于 clamp 的下限值，`vmin` 缩放只在平板以上才起作用。所以下限不是「兜底」，它就是主力设备上的真实字号，必须按手机观感来定。正文下限提到 22px，卡片内汉字、区块标题等相应上调。 |
| D-28 | 选词页栅格按视口分档 | 手机竖屏 **2 列**、480–767px **3 列**、≥768px **4 列**（上限）。用户要求「最多 4 个」，4 作为**上限**采纳；但 390px 宽下 4 列每格仅约 90px，装不下可读的汉字 + 拼音 + 两个操作按钮，所以窄屏必须更少。选词页是识别链路唯一的人工闸口，可读性优先于减少滚动。 |
| D-29 | 可点区尺寸分级 | §9 的 ≥88px 针对**听写页的儿童主操作**（孩子手指落点偏差大、且是高频盲操作）。选词页的「试听 / 读音不对」是**家长的低频次操作**，下限 48px 即可（仍高于 WCAG 的 44px）。一刀切 88px 会让选词页高度失控，反而更难用。 |
| D-23 | 「结束听写」由长按改为**点击 + 二次确认** | 真机验证发现长按会触发移动浏览器自身的长按菜单（文本选择 / 上下文菜单），导致结束根本无法生效。CSS 能压制一部分但跨浏览器不可靠；长按对长辈也不友好（不知道要按住、按不够时长），对无障碍更是死路。改为：底部小号「结束听写」→ 全宽确认面板，「继续听写」做成视觉主选项、「结束」做次要项，把误触引导向安全的一侧。防误触目标不变，且人人都懂。 |
| D-24 | 尺寸一律用 `clamp()` + 视口单位 | §9 的 ≥88px、≥72px 是**下限不是定值**。写死 px 在手机上偏小、在平板横屏上浪费。改为 `clamp(下限, 视口相对值, 上限)`，优先用 `vmin`/`dvh` 以便横竖屏都合理。 |
| D-25 | 听写页横屏改双栏 | 横屏（含平板横屏与手机横屏）竖向空间不足，单栏会把按钮挤出屏幕。改为左栏信息（序号/进度/状态）、右栏操作（三个按钮），拇指可及。 |
| D-26 | 听写页禁止页面滚动与下拉刷新 | 孩子在屏幕上乱划可能触发下拉刷新导致整页重载。听写页固定为一屏布局，并设 `overscroll-behavior: none`。 |
