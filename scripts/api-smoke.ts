...
//
// 只读地借用 lib/core/cachekey.ts 和 lib/tts/cache.ts 来验证第 6 步的缓存
...
async function main(): Promise<void> {
  ...
  // === 第 1 步：上传作业照片 ===
  console.log("\n[1/9] POST /api/worksheet —— 上传 fixtures/worksheet-01.jpg");
  ...
  const worksheetRes = await fetchJson<WorksheetPostResp>(1, `${BASE_URL}/api/worksheet`, {
    method: "POST",
    body: form,
  });
  assert(
  ...
  assert(worksheet.required.length === 14, 1, `必听词应为 14 个，实际 ${worksheet.required.length}`);
  assert(worksheet.rows.length === 10, 1, `生字行应为 10 行，实际 ${worksheet.rows.length}`);
  console.log(
  ...
  // === 第 2 步：帮我选 ===
  console.log("\n[2/9] POST /api/worksheet/:id/pick —— 每行择一");
  ...
  assert(optional.length === 10, 2, `选听词应恰好 10 个，实际 ${optional.length}`);
  const requiredTextSet = new Set(worksheet.required.map((w) => w.text));
  const overlap = optional.filter((w) => requiredTextSet.has(w.text));
  assert(overlap.length === 0, 2, `选听词与必听词有重复：${overlap.map((w) => w.text).join("、")}`);
  console.log(`  选出 ${optional.length} 个：${optional.map((w) => w.text).join("、")}`);
  console.log("  通过");

  // === 第 3 步：开一次听写会话 ===
  console.log("\n[3/9] POST /api/session —— 必听在前排序");
  const wordIds = [...worksheet.required.map((w) => w.id), ...optional.map((w) => w.id)];
  const sessionRes = await fetchJson<SessionPostResp>(3, `${BASE_URL}/api/session`, {
    method: "POST",
  ...
  assert(session.attempts.length === 24, 3, `听写总数应为 24，实际 ${session.attempts.length}`);
  const bySeq = new Map(session.attempts.map((a) => [a.seq, a]));
  for (let seq = 1; seq <= 14; seq++) {
    const a = bySeq.get(seq);
    assert(a !== undefined && a.bucket === "required", 3, `seq=${seq} 应为 required，实际 ${a?.bucket}`);
  }
  for (let seq = 15; seq <= 24; seq++) {
    const a = bySeq.get(seq);
    assert(a !== undefined && a.bucket === "optional", 3, `seq=${seq} 应为 optional，实际 ${a?.bucket}`);
  }
  console.log(`  sessionId=${session.sessionId}，总数 ${session.attempts.length}，seq 1-14 必听 / 15-24 选听`);
  console.log("  通过");

  // === 第 4 步：取第一个词的音频 ===
  console.log("\n[4/9] GET /api/audio —— 取第 1 个词");
  ...
  assert(audioRes1.status === 200, 4, `HTTP 状态应为 200，实际 ${audioRes1.status}`);
  const contentType1 = audioRes1.headers.get("content-type") ?? "";
  assert(contentType1.startsWith("audio/"), 4, `Content-Type 应是音频，实际 ${contentType1}`);
  const audioBuf1 = Buffer.from(await audioRes1.arrayBuffer());
  assert(audioBuf1.length > 1000, 4, `音频字节数应 > 1000，实际 ${audioBuf1.length}`);
  console.log(`  wordId=${firstAttempt.wordId} Content-Type=${contentType1} 字节数=${audioBuf1.length}`);
  console.log("  通过");

  // === 第 5 步：命中缓存 ===
  console.log("\n[5/9] 再取一次同一个词 —— 断言命中缓存（字节完全一致）");
  const audioRes2 = await fetch(audioUrl);
  assert(audioRes2.status === 200, 5, `HTTP 状态应为 200，实际 ${audioRes2.status}`);
  const audioBuf2 = Buffer.from(await audioRes2.arrayBuffer());
  const cacheHit = audioBuf1.equals(audioBuf2);
  console.log(`  第4步 ${audioBuf1.length} 字节 vs 第5步 ${audioBuf2.length} 字节，完全一致：${cacheHit}`);
  assert(cacheHit, 5, "两次响应字节不一致，说明没有命中缓存");
  console.log("  通过");

  // === 第 6 步：读音修正 -> 缓存失效 ===
  console.log("\n[6/9] PATCH /api/word/:id/tts-text —— 改读音后验证旧缓存失效");
  ...
  assert(getCached(oldKey) !== null, 6, "PATCH 之前旧缓存应该已经存在（第 4 步应已写盘）");
  ...
  assert(getCached(oldKey) === null, 6, "PATCH 之后旧缓存应该被清掉，但磁盘上仍然存在");

  const audioRes3 = await fetch(audioUrl);
  assert(audioRes3.status === 200, 6, `HTTP 状态应为 200，实际 ${audioRes3.status}`);
  ...
  console.log(
    `  字节对比：第4步 ${audioBuf1.length} 字节 vs 第6步 ${audioBuf3.length} 字节，是否不同：${bytesDiffer}`
  );
  if (!bytesDiffer) {
    console.log(
  ...
  assert(newCacheWritten, 6, "改读音后重新合成应该写入一份新的缓存文件，但没有观察到");
  console.log("  通过");

  // === 第 7 步：更新 attempt ===
  console.log("\n[7/9] PATCH /api/attempt/:id —— 一个 skipped，一个 hintLevel=3");
  ...
  const sessionFullRes = await fetchJson<SessionFullResp>(7, `${BASE_URL}/api/session/${session.sessionId}`);
  assert(
  ...
  assert(skipAttemptId !== undefined, 7, `找不到 seq=${skipAttempt.seq} 对应的 attemptId`);
  assert(hintAttemptId !== undefined, 7, `找不到 seq=${hintAttempt.seq} 对应的 attemptId`);

  const skipPatch = await fetchJson<{ ok: true }>(7, `${BASE_URL}/api/attempt/${skipAttemptId}`, {
    method: "PATCH",
    headers: { "Content-Type": "application/json" },
  ...
  const hintPatch = await fetchJson<{ ok: true }>(7, `${BASE_URL}/api/attempt/${hintAttemptId}`, {
    method: "PATCH",
    headers: { "Content-Type": "application/json" },
  ...
  // === 第 8 步：结束听写 ===
  console.log("\n[8/9] POST /api/session/:id/finish");
  const finishRes = await fetchJson<FinishResp>(8, `${BASE_URL}/api/session/${session.sessionId}/finish`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({}),
  });
  ...
  assert(finishRes.body.skipped === 1, 8, `skipped 应为 1，实际 ${finishRes.body.skipped}`);
  assert(finishRes.body.hintCount === 1, 8, `hintCount 应为 1，实际 ${finishRes.body.hintCount}`);
  console.log(
